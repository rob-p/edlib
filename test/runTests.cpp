...
void fillRandomlyDNA(char* seq, int seqLength, int alphabetLength) {
  char chars[] = {'A', 'C', 'G', 'G'};
    for (int i = 0; i < seqLength; i++)
      seq[i] = chars[rand() % alphabetLength];
}
...
bool runRandomTestsAlignerEngine(int numTests, int rl, EdlibAlignMode mode, bool findAlignment) {
    ...
    for (int i = 0; i < numTests; i++) {
        ...
        fillRandomlyDNA(target, targetLength, alphabetLength);
        ...
        start = clock();
        ae(query, queryLength, target, targetLength,
           edlibNewAlignConfig(-1, mode, findAlignment ? EDLIB_TASK_PATH : EDLIB_TASK_DISTANCE));
        auto& result = ae.result();
        timeEdlib += clock() - start;
        if (findAlignment){//}result.alignment) {
            if (!checkAlignment(query, queryLength, target, targetLength,
                                result.editDistance, result.endLocations[0], mode,
                                const_cast<unsigned char*>(result.alignment.data()), result.alignmentLength)) {
                failed = true;
                printf("Alignment is not correct\n");
            }
            int alignmentStart = getAlignmentStart(const_cast<unsigned char*>(result.alignment.data()), result.alignmentLength,
        ...
        } else if (result.editDistance == -1 && !(result.endLocations.size() == 0)) {
            failed = true;
